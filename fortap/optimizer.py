...
def warmup_cosine(x, warmup=0.002):
    if x < warmup:
        return x / warmup
    return (torch.cos(math.pi * x) + 1.0) * 0.5
...
class BertAdam(Optimizer):
    def __init__(self, params, lr, warmup=-1, t_total=-1, schedule='warmup_linear',
        ...
        if not 0.0 <= warmup < 1.0 and not warmup == -1:
            raise ValueError("Invalid warmup: {} shoule be in [0,1] or -1".format(warmup))
        if not 0.0 <= b1 < 1.0:
            raise ValueError("Invalid b1 parameter: {} should be in [0,1]".format(b1))
        if not 0.0 <= b2 < 1.0:
            raise ValueError("Invalid b2 parameter: {} should be in [0,1]".format(b2))
        if not e >= 0.0:
            raise ValueError("Invalid epsilon parameter: {} should be >= 0.0".format(e))
        defaults = dict(lr=lr, schedule=schedule, warmup=warmup, t_total=t_total, 
                        b1=b1, b2=b2, e=e, weight_decay_rate=0.01, max_grad_norm=max_grad_norm)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['t_total'] != -1:
                    ...
                    lr_scheduled = schedule_fct(state['step']/group['t_total'], group['warmup'])
                else:
